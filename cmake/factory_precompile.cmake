...
function(generate_factory_precompile_sources TARGET_NAME SOURCE_DIR)
  ...
  foreach(HEADER ${FACTORY_HEADERS})
    ...
    # Pattern: namespace eicrecon { class FactoryName_factory : public
    # JOmniFactory<FactoryName_factory, ConfigType> } First check if we're in
    # eicrecon namespace
    ...
    if(CMAKE_MATCH_1 AND CMAKE_MATCH_2)
      ...
      if(HAS_NAMESPACE)
        ...
        # Config is also in eicrecon namespace ONLY if it's defined in the
        # factory header or algorithm header within the eicrecon namespace block
        ...
        if(CONFIG_HEADERS)
          ...
          # No config header found, assume it's in eicrecon namespace (like
          # NoConfig)
          set(CONFIG_TYPE "eicrecon::${CONFIG_TYPE}")
        endif()
      endif()
    ...
    # Add factory header include for .cc file using path relative to
    # src/factories/
...
${EXPLICIT_INSTANTIATIONS}")
  ...
  # For header-only factory plugins, create a small library to compile the
  # precompiled instantiations The library will be linked by any plugin that
  ...
  if(NOT TARGET ${PRECOMPILE_LIB})
    ...
    # Use the standard plugin macros to add external dependencies These will
    # no-op if already found, or find and link the packages
    ...
    # Ensure Acts version macros are defined (needed for conditional
    # compilation)
    if(Acts_FOUND)
    ...
    # Link against ALL algorithm and service libraries to ensure all includes
    # and dependencies are available This is necessary because factory headers
    # can transitively include any algorithm or service Use a list to make it
    # easier to maintain
