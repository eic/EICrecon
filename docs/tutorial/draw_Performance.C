//Code to plot Basic properties after simulation and reconstruction
// Shyam Kumar; INFN Bari, Italy; shyam.kumar@ba.infn.it; shyam055119@gmail.com
 ...
 //  cout<<"=====Event No. "<<count<<"============="<<endl;
   ...
   //  cout<<" PDG: "<<pdg[iParticle]<<" Status: "<<status[iParticle]<<" Pt: "<<sqrt(px_mc[iParticle]*px_mc[iParticle]+py_mc[iParticle]*py_mc[iParticle])<<endl;
    ...
    count++;
      ...
      heff_pnum->GetYaxis()->SetTitle("#eta_{mc}");
      heff_pnum->GetXaxis()->SetTitle("p_{mc}");
      heff_pnum->GetZaxis()->SetRangeUser(0.,1.1);
      
      heff_ptnum->GetYaxis()->SetTitle("#eta_{mc}");
      heff_ptnum->GetXaxis()->SetTitle("pt_{mc}");
