...
public:
  ...
  void Process(int64_t run_number, uint64_t event_number) {
    std::tie(m_trajectories_output(), m_parameters_output(), m_tracks_output(),
             m_acts_trajectories_output(), m_acts_tracks_output()) =
        m_algo->process(*m_measurements_input(), *m_parameters_input());
  }
};
