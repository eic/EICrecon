...
class ParticlesWithPID_factory
    : public JOmniFactory<ParticlesWithPID_factory, ParticlesWithPIDConfig> {
...
private:
  ...
  ParameterRef<double> m_momentumRelativeTolerance{this, "momentumRelativeTolerance",
...
public:
  ...
  void Process(int64_t run_number, uint64_t event_number) {
    std::tie(m_particles_output(), m_particles_assoc_output(), m_particle_id_output()) =
        m_algo.process(m_mc_particles_input(), m_tracks_input(), m_drich_particle_id_input());
  }
};
