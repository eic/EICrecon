...
//-----------------------------------------------------------------------------
void eicrecon::IrtCherenkovParticleID_factory::BeginRun(
...
void eicrecon::IrtCherenkovParticleID_factory::Process(const std::shared_ptr<const JEvent>& event) {
  ...
  std::map<std::string, const edm4eic::TrackSegmentCollection*>
      charged_particles; // map : radiator_name -> collection of TrackSegments
  int tag_num = 0;
  while (tag_num < richgeo::nRadiators) {
    ...
    if (radiator_id >= 0 && radiator_id < richgeo::nRadiators)
      charged_particles.insert({richgeo::RadiatorName(radiator_id),
                                (event->GetCollection<edm4eic::TrackSegment>(input_tag))});
    else
      m_log->error("Unknown input RICH track collection '{}'", input_tag);
  }

  // get merged track propagation
  if (GetInputTags()[tag_num].find("Merge") != std::string::npos)
    charged_particles.insert(
        {"Merged", (event->GetCollection<edm4eic::TrackSegment>(GetInputTags()[tag_num++]))});
  else {
  ...
  const auto* hit_assocs =
      (event->GetCollection<edm4eic::MCRecoTrackerHitAssociation>(GetInputTags()[tag_num++]));
