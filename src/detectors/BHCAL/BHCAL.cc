...
void InitPlugin(JApplication* app) {
  ...
  if (UseSectorIndexedBHCalReadout(app)) {
    HcalBarrel_adjacencyMatrix =
        ...
        "  + min("
        "      abs((sector_1 - sector_2) * (2 * 5) + (floor(tower_1 / 24) - floor(tower_2 / 24)) * "
        "5 + fmod(tile_1, 5) - fmod(tile_2, 5)),"
        "      (32 * 2 * 5) - abs((sector_1 - sector_2) * (2 * 5) + (floor(tower_1 / 24) - "
        "floor(tower_2 / 24)) * 5 + fmod(tile_1, 5) - fmod(tile_2, 5))"
        "    )"
        ") == 1";
  }
