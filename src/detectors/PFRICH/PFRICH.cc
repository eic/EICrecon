...
extern "C" {
  void InitPlugin(JApplication *app) {
    ...
    if (dd4hepGeo->world().children().contains("PFRICH")) {
      printf("@@@ PFRICH geometry found!\n");
      auto actsGeo = richGeoSvc->GetActsGeo("PFRICH");
    ...
    // PFRICH IRT debugging algorithm;
    app->Add(new JOmniFactoryGeneratorT<IrtDebugging_factory>(
	    ...
	    "PFRICHAerogelTracks", "PFRICHHits"
          },
          {"IrtDebugInfoTables"},
          //irt_cfg,
          app
