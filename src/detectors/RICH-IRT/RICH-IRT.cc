    ...
    // Check command line options; expect a "config" key with a JSON configuration file name;
    {
      ...
      // to be present; FIXME: add warning / error printouts;
      {
        ...
        // Import Cherenkov detector optics configuration file;
        {
          ...
          config.m_irt_geometry = dynamic_cast<IRT2::CherenkovDetectorCollection*>(
              foptics->Get("IRT2::CherenkovDetectorCollection"));
          ...
          // Eventually define the factories and collections to be used;
          {
            ...
            app->Add(new JOmniFactoryGeneratorT<IrtInterface_factory>(
                ...
                {"MCParticles", "ReconstructedChargedWithoutPIDParticles",
                 "ReconstructedChargedWithoutPIDParticleAssociations", RICHtracks.Data(),
                 (RICHstr + "Hits").Data()},
                {(RICHstr + "IrtRadiatorInfo").Data(), (RICHstr + "IrtParticles").Data(),
