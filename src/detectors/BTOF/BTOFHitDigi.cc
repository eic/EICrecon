...
BTOFHitDigi::execute(const edm4hep::SimTrackerHitCollection* simhits) {
  ...
  for (const auto& [id, ixs] : merge_map) {
    ...
    auto localPos_hit = _neighborFinder.global2Local(
        dd4hep::Position(truePos.x / 10., truePos.y / 10., truePos.z / 10.));
    ...
    for (size_t i = 0; i < ixs.size(); ++i) {
      ...
      auto neighbours = _neighborFinder.findAllNeighborInSensor(
          mid); // Accessing NeighbourFinder through DetPosProcessor

      for (const auto& neighbour : *neighbours) {

        auto localPos_neighbour = _neighborFinder.cell2LocalPosition(neighbour);

        double charge = sum_charge *
                        _integralGaus(localPos_hit.x(), m_cfg.sigma_sharingx,
                                      localPos_neighbour.x() - 0.5 * cellDimension[0],
                                      localPos_neighbour.x() + 0.5 * cellDimension[0]) *
