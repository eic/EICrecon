...
void TofEfficiency_processor::InitWithGlobalRootLock() {
  ...
  m_th2_btof_phiz = new TH2F("btof_phiz", "Hit position for Barrel TOF", 100, phi_limit_min,
                             phi_limit_max, 100, z_limit_min, z_limit_max);
  m_th2_ftof_rphi = new TH2F("ftof_rphi", "Hit position for Forward TOF", 100, r_limit_min,
                             r_limit_max, 100, phi_limit_min, phi_limit_max);
  m_th2_btof_phiz->SetDirectory(m_dir_main);
  m_th2_ftof_rphi->SetDirectory(m_dir_main);

  m_tntuple_track = new TNtuple(
