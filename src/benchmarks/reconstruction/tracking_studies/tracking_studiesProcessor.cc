...
void tracking_studiesProcessor::InitWithGlobalRootLock() {
  ...
  hThetaResoVsEta = new TH2D("hThetaResoVsEta", "", 400, -4, 4, 200, -0.1, 0.1);
  hThetaResoVsP   = new TH2D("hThetaResoVsP", "", 500, 0, 10, 200, -0.1, 0.1);
  hPhiResoVsEta   = new TH2D("hPhiResoVsEta", "", 400, -4, 4, 200, -0.1, 0.1);
  hPhiResoVsP     = new TH2D("hPhiResoVsP", "", 500, 0, 10, 200, -0.1, 0.1);
  ...
  auto mRICH_center_Bounds =
      std::make_shared<Acts::RadialBounds>(mRICH_center_MinR, mRICH_center_MaxR);
  auto mRICH_center_Trf = transform * Acts::Translation3(Acts::Vector3(0, 0, mRICH_center_Z));
  m_mRICH_center_surface =
  ...
  for (const auto& traj : acts_results) {
    ...
    // projection to special surfaces
    {
      ...
      if(!projection_point_DIRC) {
        ...
        // pos.y, pos.z, length);
        // cout << "\tdRICH_projection pos: " << proj_pos.x << " " << proj_pos.y << " " << proj_pos.z << " " << proj_length
        //      << endl;
        ...
        for (const auto hit : event->Get<edm4hep::SimTrackerHit>("DIRCBarHits")) {
          ...
          // cout << "\tdRICH_hit pos: " << hit_pos.x << " " << hit_pos.y << " " << hit_pos.z << endl;
