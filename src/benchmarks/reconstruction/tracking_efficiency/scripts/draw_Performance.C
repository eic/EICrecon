  ...
  while (myReader.Next()) {
    ...
    for (int iParticle = 0; iParticle < charge.GetSize(); ++iParticle) {
      ...
      if (status[iParticle] == 1) {
        pmc = sqrt(px_mc[iParticle] * px_mc[iParticle] + py_mc[iParticle] * py_mc[iParticle] +
                   pz_mc[iParticle] * pz_mc[iParticle]);
        Double_t ptmc =
            sqrt(px_mc[iParticle] * px_mc[iParticle] + py_mc[iParticle] * py_mc[iParticle]);
        ...
        if (px_rec.GetSize() == 1) {
          Double_t prec =
              sqrt(px_rec[iParticle] * px_rec[iParticle] + py_rec[iParticle] * py_rec[iParticle] +
                   pz_rec[iParticle] * pz_rec[iParticle]);
          Double_t ptrec =
              sqrt(px_rec[iParticle] * px_rec[iParticle] + py_rec[iParticle] * py_rec[iParticle]);
          Double_t etarec =
              -1.0 * TMath::Log(TMath::Tan((TMath::ACos(pz_rec[iParticle] / prec)) / 2));
