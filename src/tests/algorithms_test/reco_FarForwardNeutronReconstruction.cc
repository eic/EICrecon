...
TEST_CASE("the cluster merging algorithm runs", "[FarForwardNeutronReconstruction]") {
  ...
  std::shared_ptr<spdlog::logger> logger =
      spdlog::default_logger()->clone("FarForwardNeutronReconstruction");
  ...
  std::cout << "E, px, py, pz = " << (*neutroncand_coll)[0].getEnergy() << "  "
            << (*neutroncand_coll)[0].getMomentum().x << "  "
            << (*neutroncand_coll)[0].getMomentum().y << "  "
            << (*neutroncand_coll)[0].getMomentum().z << std::endl;
