...
using FarForwardNeutronReconstructionAlgorithm = algorithms::Algorithm<
    algorithms::Input<const edm4eic::ClusterCollection, std::optional<edm4eic::ClusterCollection>>,
...
public:
  FarForwardNeutronReconstruction(std::string_view name)
      : FarForwardNeutronReconstructionAlgorithm{
            name,
            {"inputClustersHcal", "inputClustersEcal"},
            {"outputNeutrons"},
            "Merges all HCAL (and optionally also ECAL) clusters in a collection into a neutron "
            "candidate"} {}
