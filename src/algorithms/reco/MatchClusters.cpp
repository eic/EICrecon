...
//  DataHandle<edm4eic::MCRecoParticleAssociationCollection> m_outputParticlesAssoc{"ReconstructedParticlesAssoc", Gaudi::DataHandle::Writer, this};
    ...
    public:
        ...
        ParticlesWithAssociation *execute(
            ...
            std::map<int, edm4eic::Cluster> clusterMap;
            // TODO auto clusterMap = indexedClusters(m_inputClustersCollections, m_inputClustersAssocCollections);
            ...
            for (const auto &inpart: inparts) {
                m_log->debug(" --> Processing charged particle {}, PDG {}, energy {}", inpart->getObjectID().index, inpart->getPDG(), inpart->getEnergy());
            ...
            for (const auto &[mcID, clus]: clusterMap) {
                ...
                if (m_log->level() <= spdlog::level::debug) {
                    ...
                    m_log->debug("   --> found matching associated mcparticle with PDG: {}, energy: {}", pdg, mc->getEnergy());
                }

                // Reconstruct our neutrals and add them to the list
                const auto outpart = reconstruct_neutral(clus, mass, pdg);
                m_log->debug(" --> Reconstructed neutral particle with PDG: {}, energy: {}", outpart.getPDG(), outpart.getEnergy());
