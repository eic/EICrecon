...
void SecondaryVerticesHelix::process(const SecondaryVerticesHelix::Input& input,
  ...
  std::vector<Helix> hVec;  hVec.clear();
  std::vector<unsigned int> indexVec;  indexVec.clear();
  ...
  if( hVec.size() != indexVec.size() ) return;
  ...
  for (unsigned int i1 = 0; i1 < hVec.size(); ++i1) {
    for (unsigned int i2 = i1 + 1; i2 < hVec.size(); ++i2) {
       ...
       if ( !(m_cfg.unlikesign && p1.getCharge() + p2.getCharge() == 0) ) continue;
       ...
       double e1 = std::hypot(edm4hep::utils::magnitude(h1MomAtDca), particleSvc.particle(p1.getPDG()).mass);
       double e2 = std::hypot(edm4hep::utils::magnitude(h2MomAtDca), particleSvc.particle(p2.getPDG()).mass);
       double pairE = e1+e2;
       ...
       edm4hep::Vector3f decayL(dL.x*edm4eic::unit::cm, dL.y*edm4eic::unit::cm, dL.z*edm4eic::unit::cm);
       double dca2pv = edm4hep::utils::magnitude(decayL) * sin(angle);
       if(dca2pv > m_cfg.maxDca) continue;
