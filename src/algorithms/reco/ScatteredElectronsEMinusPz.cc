...
/**
   ...
   * @param logger
   */
void ScatteredElectronsEMinusPz::init(std::shared_ptr<spdlog::logger>& logger) { m_log = logger; }
...
ScatteredElectronsEMinusPz::execute(const edm4eic::ReconstructedParticleCollection* rcparts,
  ...
  m_log->trace("We have {} candidate electrons", rcele->size());
  ...
  for (const auto& e : *rcele) {
    ...
    m_log->trace("\tScatteredElectron has Pxyz=( {}, {}, {} )", e.getMomentum().x,
                 e.getMomentum().y, e.getMomentum().z);
  ...
  for (auto kv = scatteredElectronsMap.rbegin(); kv != scatteredElectronsMap.rend(); ++kv) {
    ...
    if (first) {
      ...
      m_log->trace("\tScatteredElectron has Pxyz=( {}, {}, {} )", kv->second.getMomentum().x,
