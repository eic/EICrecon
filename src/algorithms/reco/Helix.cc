...
void Helix::setParameters(const edm4eic::TrackParameters& trk, const double b_field) {
  ...
  setParameters(mom*edm4eic::unit::GeV/dd4hep::GeV, pos*edm4eic::unit::mm/edm4eic::unit::cm, b_field, charge);
}

void Helix::setParameters(const edm4hep::Vector3f& p, const edm4hep::Vector3f& o, const double B, const int q)
{
    mH = (q*B <= 0) ? 1 : -1;
    if(p.y == 0 && p.x == 0)
    ...
    double curvature_val = fabs((dd4hep::c_light*dd4hep::nanosecond/dd4hep::meter*q*B/dd4hep::tesla)/
		           (edm4hep::utils::magnitude(p)/dd4hep::GeV*mCosDipAngle)/dd4hep::meter);		      
    setCurvature(curvature_val);
}

void Helix::setParameters(double c, double dip, double phase,
...
double Helix::pathLength(const edm4hep::Vector3f& r,
...
void Helix::Print() const
{
    std::cout << "("
	      ...
	      << "phase = "      << mPhase     << ", "  
	      << "h = "          << mH         << ", "    
	      << "origin = "     << mOrigin.x << " " << mOrigin.y << " " << mOrigin.z << ")"
	      << std::endl;
}
...
edm4hep::Vector3f Helix::momentum(double B) const
{
	...
	return(edm4hep::Vector3f(0,0,0));
        ...
        double pt = edm4eic::unit::GeV*fabs(dd4hep::c_light*dd4hep::nanosecond/dd4hep::meter*B/dd4hep::tesla)/(fabs(mCurvature)*dd4hep::meter);
...
int Helix::charge(double B) const
{
    return (B > 0 ? -mH : mH);
}
...
double Helix::geometricSignedDistance(double x, double y)  
{
	...
	momVec = this->momentumAt(thePath,1./dd4hep::tesla); // Don't care about Bmag.  Helicity is what matters.
	momVec.z = 0;
    }
