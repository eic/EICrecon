...
void LGADHitClustering::process(const LGADHitClustering::Input& input,
  ...
  for (const auto& sensor : hitsBySensors) {
    ...
    if (!id)
      error("Failed to find cellID for cluster center! Does your detector have hole in the middle "
            "or somehow concave in shape? This class doesn't work with concave detector.");
