...
void IrtInterface::JsonParser(void) {
  ...
  if (jptr->find("DefaultSinglePhotonThetaResolution") != jptr->end())
    reco->SetDefaultSinglePhotonThetaResolution(
        (*jptr)["DefaultSinglePhotonThetaResolution"].template get<double>());
  ...
  if (jptr->find("SensorActiveAreaPixellation") != jptr->end())
    reco->SetSensorActiveAreaPixellation(
        (*jptr)["SensorActiveAreaPixellation"].template get<int>());

  //-reco->SetSinglePhotonTimingResolution(0.030); // default: 0.050 (50ps);
  if (jptr->find("SinglePhotonTimingResolution") != jptr->end())
    reco->SetSinglePhotonTimingResolution(
        (*jptr)["SinglePhotonTimingResolution"].template get<double>());
