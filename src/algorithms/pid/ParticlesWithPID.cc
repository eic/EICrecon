...
void ParticlesWithPID::init(std::shared_ptr<spdlog::logger> logger) { m_log = logger; }

ParticlesWithAssociation ParticlesWithPID::process(
    const edm4hep::MCParticleCollection* mc_particles, const edm4eic::TrackCollection* tracks,
  ...
  std::vector<bool> mc_prt_is_consumed(mc_particles->size(),
                                       false); // MCParticle is already consumed flag
  ...
  for (const auto& track : *tracks) {
    ...
    for (const auto& trk : trajectory.getTrackParameters()) {
      ...
      m_log->debug(" Track : {:<4} {:<8.3f} {:<8.3f} {:<8.2f} {:<4}", trk.getObjectID().index,
      ...
      for (size_t ip = 0; ip < mc_particles->size(); ++ip) {
        ...
        m_log->trace("  MCParticle with id={:<4} mom={:<8.3f} charge={}",
                     mc_part.getObjectID().index, edm4hep::utils::magnitude(p),
                     mc_part.getCharge());
        ...
        bool is_matching = dp_rel < m_cfg.momentumRelativeTolerance && deta < m_cfg.etaTolerance &&
        ...
        m_log->trace("    Decision: {}  dp: {:.4f} < {}  &&  d_eta: {:.6f} < {}  && d_sin_phi: "
                     "{:.4e} < {:.4e} ",
                     is_matching ? "Matching" : "Ignoring", dp_rel, m_cfg.momentumRelativeTolerance,
                     deta, m_cfg.etaTolerance, dsphi, sinPhiOver2Tolerance);

        if (is_matching) {
          const double delta = std::hypot(dp_rel / m_cfg.momentumRelativeTolerance,
                                          deta / m_cfg.etaTolerance, dsphi / sinPhiOver2Tolerance);
          if (delta < best_delta) {
            best_match = ip;
            best_delta = delta;
      ...
      rec_part.setType(
          static_cast<int16_t>(best_match >= 0 ? 0 : -1)); // @TODO: determine type codes
      ...
      if (success)
        m_log->trace(
            "      true PDG vs. CherenkovPID PDG: {:>10} vs. {:<10}", best_pid,
      ...
      if (best_match >= 0) {
        ...
        if (m_log->level() <= spdlog::level::debug) {
          ...
          m_log->debug(" Assoc: id={} SimId={} RecId={}", rec_assoc.getObjectID().index,
                       rec_assoc.getSim().getObjectID().index,
                       rec_assoc.getSim().getObjectID().index);

          m_log->trace(
              " Assoc PDGs: sim.PDG | rec.PDG | rec.particleIDUsed.PDG = {:^6} | {:^6} | {:^6}",
              rec_assoc.getSim().getPDG(), rec_assoc.getRec().getPDG(),
              rec_assoc.getRec().getParticleIDUsed().isAvailable()
                  ? rec_assoc.getRec().getParticleIDUsed().getPDG()
                  : 0);
        }
      } else {
...
void ParticlesWithPID::tracePhiToleranceOnce(const double sinPhiOver2Tolerance,
                                             double phiTolerance) {
  ...
  std::call_once(do_it_once, [this, sinPhiOver2Tolerance, phiTolerance]() {
    m_log->trace("m_cfg.phiTolerance: {:<8.4f} => sinPhiOver2Tolerance: {:<8.4f}",
                 sinPhiOver2Tolerance, phiTolerance);
  });
}
...
/* link PID objects to input particle
     ...
     */
bool ParticlesWithPID::linkCherenkovPID(edm4eic::MutableReconstructedParticle& in_part,
  ...
  m_log->trace("Input particle: (eta,phi) = ( {:>5.4}, {:>5.4} deg )", in_part_eta,
  ...
  for (std::size_t in_pid_idx = 0; in_pid_idx < in_pids.size(); in_pid_idx++) {
    ...
    // logging
    m_log->trace("  - (eta,phi) = ( {:>5.4}, {:>5.4} deg ),  match_dist = {:<5.4}{}", in_track_eta,
  ...
  m_log->trace("  => best match: match_dist = {:<5.4} at idx = {}", closest_prox_match.match_dist,
