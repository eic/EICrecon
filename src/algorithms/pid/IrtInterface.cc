...
IrtInterface::~IrtInterface() {
  ...
  if (!m_InstanceCounters[m_OutputFileName]) {
    ...
    if (m_ReconstructionFactory)
      m_ReconstructionFactory->DisplayStandardPlots("Track / event level plots", -1265, 10, 625,
                                                    1115);
...
void IrtInterface::init(DD4hep_service& dd4hep_service, IrtConfig& config,
...

  {
    ...
    if (!m_InstanceCounters[m_OutputFileName]) {
      ...
      m_EventBranches[m_OutputFileName] = m_EventTrees[m_OutputFileName]->Branch(
          "e", "IRT2::CherenkovEvent", 0 /*&m_Event*/, 16000, 2);
    } //if
    ...
    if (jptr->find("Photosensor") != jptr->end()) {
      ...
      if (jpref.find("quantum-efficiency") != jpref.end()) {
        ...
        pd->SetQE(
            ...
            new G4DataInterpolation(qePhotonEnergy.data(), qeData.data(), qeEntries /*, 0.0, 0.0*/),
            qemax ? 1.0 / qemax : 1.0);
        // FIXME: 100 hardcoded;
...
void IrtInterface::process(const IrtInterface::Input& input,
  ...
  for (auto mchit : *in_sim_hits) {
    ...
    if (m_irt_det->m_PhotonDetectors.size() != 1)
      continue;
    auto pd = m_irt_det->m_PhotonDetectors[0];
