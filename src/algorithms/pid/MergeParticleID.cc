    ...
    )
{
  ...
  // print log for a given hypothesis
  auto PrintHypothesis = [this] (edm4eic::CherenkovParticleIDHypothesis hyp) {
    m_log->trace("      PDG = {:<6} weight = {:<10.8}  NPE = {:<10.8}", hyp.PDG, hyp.weight, hyp.npe);
  };
  ...
  for(auto [id, in_particle_pids] : charged_particle_2_pids) {
    ...
    for(auto [pdg,out_hyp] : pdg_2_hyp) out_pid.addToHypotheses(out_hyp);
    m_log->trace("    => merged hypothesis weights:");
    for(auto out_hyp : out_pid.getHypotheses()) PrintHypothesis(out_hyp);
