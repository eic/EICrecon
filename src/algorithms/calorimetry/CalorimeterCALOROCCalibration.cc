...
double CalorimeterCALOROCCalibration::_energyCor(double referencePos, double energy, double z) const {
    double length = std::abs(referencePos - z);
    double factor = m_cfg.attenuationParameters[0] * std::exp(-length/m_cfg.attenuationParameters[1]) + (1 - m_cfg.attenuationParameters[0]) * std::exp(-length/m_cfg.attenuationParameters[2]);
    return energy/factor;
}

void CalorimeterCALOROCCalibration::init() {

  if(m_cfg.attenuationReferencePositionNamePos.empty() || m_cfg.attenuationReferencePositionNameNeg.empty()) {
    error("You MUST provide the name of reference positions for attenuation correction");
  } 

  m_reference_z_p = m_geo.detector()->constant<double>(m_cfg.attenuationReferencePositionNamePos) * edm4eic::unit::mm / dd4hep::mm;
  m_reference_z_n = m_geo.detector()->constant<double>(m_cfg.attenuationReferencePositionNameNeg) * edm4eic::unit::mm / dd4hep::mm;
...
double CalorimeterCALOROCCalibration::_timeWalkCorrection(double toa, double lowGainADC) const {
  if(static_cast<double>(lowGainADC) - m_cfg.timeWalkCorrectionParameters[2] > 0)
    return toa - (m_cfg.timeWalkCorrectionParameters[1] * pow(static_cast<double>(lowGainADC) - m_cfg.timeWalkCorrectionParameters[2], 
        m_cfg.timeWalkCorrectionParameters[3]) + m_cfg.timeWalkCorrectionParameters[0]);
  else return toa;
}

void CalorimeterCALOROCCalibration::process(const CalorimeterCALOROCCalibration::Input& input,
  ...
  std::unordered_map<dd4hep::rec::CellID, size_t> cellID2PulsesNID, cellID2PulsesPID, cellID2ADCNID, cellID2ADCPID;
  ...
  for(size_t i = 0; i < ADCNs -> size(); ++i) {
   ...
   for (const auto& pulseP: *pulsesP) {
      ...
      std::vector<int> key;
      for(const auto idx : m_field_idxs) key.push_back(static_cast<int>(id_dec->get(cellID, idx)));
      ...
      auto recohit = recohits->create(cellID, corE, 0, time, 0, position, dimension, sid,
                                      lid, local_position);
