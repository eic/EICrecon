...
//++++++++++++++++++++++++++++++++++  Version 4  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//++++++++++++++     (Scfi hits are grouped with Scfi alone and same for Img.    +++++++++++++++++++++++++
//+++++++++++     and then there is a cross system Clustering        ++++++++++++++++++
...
//           //       hit.getLocal().y, hit.getLocal().z, hit.getPosition().x, hit.getPosition().y,
//           //       hit.getPosition().z);
//           debug("T{}_{} [label=\"hit {}\", group=T{}];", i, idx, idx, i);
//         }
//         for (auto& e : group_edges[i]) {
//               debug("T{}_{} -- T{}_{};", i, e.first, i, e.second);
//           }
...
//   auto final_clusters = mergeCrossSystemClusters(clusters_by_system, all_clusters, *hits);

//   // writeDotClusters(all_clusters, clusters_by_system, final_clusters, cross_system_edges, *hits, sys_field,
//   //                          "clusters.dot");

//   debug("Outputting {} final proto-clusters", final_clusters.size());
//   for (size_t i = 0; i < final_clusters.size(); ++i) {
//         debug("  proto-cluster {} has {} hits", i, final_clusters[i].size());
//   }
...
// ImagingTopoCluster :: mergeCrossSystemClusters(
//     const std::map<int, std::vector<size_t>>& clusters_by_system,
...
//     // If only one subsystem present : no merging
//     if (!(have_Img && have_ScFi)) {
//         for (auto& cl : all_clusters) {
//             debug("Only one system present, adding cluster with {} hits", cl.size());
//             result.push_back(cl);
//         }
//         return result;
//     }
...
//     // Track ScFi clusters that have already been merged into an Imaging cluster
//     std::set<size_t> used_ScFi;

//     // Loop over each Imaging cluster
//     for (size_t idx_Img : Img) {
//         const auto& cl_Img = all_clusters[idx_Img];

//         debug("Processing Imaging cluster {} with {} hits", idx_Img, cl_Img.size());

//         // merged cluster with Imaging hits
//         std::vector<size_t> merged_cl = cl_Img;

//         // loop over all ScFi clusters
//         for (size_t idx_ScFi : ScFi) {
//             debug("  Checking ScFi cluster {} with {} hits", idx_ScFi, all_clusters[idx_ScFi].size());

//             if (used_ScFi.count(idx_ScFi)) {
//                 debug("    Skipping 105 cluster {} (already taken)", idx_ScFi);
//                 continue;   // skip already locked ScFi cluster
//             }

//             const auto& cl_ScFi = all_clusters[idx_ScFi];
//             bool neighbor_found = false;

//             // cross system neighbour check
//             for (auto h1 : cl_Img) {
//                 for (auto h2 : cl_ScFi) {
//                     if (cross_system_is_neighbour(hits[h1], hits[h2])) {
//                         debug("Cross sytem Neighbor hit found: ImgHit={} ScFiHit={}",h1, h2);
//                         neighbor_found = true;
//                         break;
//                     }
//                 }
...
//             if (neighbor_found) {
//                 debug("    Neighbor found! Merging Imaging cluster {} with 105 cluster {}", idx_Img, idx_ScFi);

//                 // Add ScFi cluster hits to merged cluster
//                 merged_cl.insert(merged_cl.end(), cl_ScFi.begin(), cl_ScFi.end());
//                 // Mark ScFi cluster as taken
//                 used_ScFi.insert(idx_ScFi);
//                 debug("    105 cluster {} is now locked", idx_ScFi);
//             } else {
//                 debug("    No neighbor found for 105 cluster {}", idx_ScFi);
//             }
//         }

//         debug("Merged Imaging cluster {} has total {} hits after checking ScFi clusters", idx_Img, merged_cl.size());
//         result.push_back(std::move(merged_cl));
//     }

//     // ScFi clusters that were NOT merged into any Imaging cluster
//     for (size_t idx_ScFi : ScFi) {
//         if (!used_ScFi.count(idx_ScFi)) {
//             debug("Remaining ScFi cluster {} with {} hits added", idx_ScFi, all_clusters[idx_ScFi].size());
//             result.push_back(all_clusters[idx_ScFi]);
//         }
//     }
...
//++++++++++++++++++++++++++++++++++  Version 1  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//++++++++++++++     (Scfi hits are grouped with Scfi alone and same for Img.    +++++++++++++++++++++++++
//+++++++++++           Cross system clustering is not allowed)                    ++++++++++++++++++
...
//       auto* sys_field = m_idSpec.field("system");
//       int sysid = sys_field ? sys_field->value(hit.getCellID()) : -1;

//       debug("  hit {} -> system = {},energy = {:.6f}, layer = {}, sector = {}, local = ({:.2f}, {:.2f}, {:.2f}), global = ({:.2f}, {:.2f}, {:.2f})",
//             idx, sysid, hit.getEnergy(), hit.getLayer(), hit.getSector(), hit.getLocal().x,
//             hit.getLocal().y, hit.getLocal().z, hit.getPosition().x, hit.getPosition().y,
//             hit.getPosition().z);
...
//     switch (m_cfg.sameLayerMode) {
//     case ImagingTopoClusterConfig::ELayerMode::xy:
//       return (std::abs(h1.getPosition().x - h2.getPosition().x) <= sameLayerDistXY[0]) &&
//              (std::abs(h1.getPosition().y - h2.getPosition().y) <= sameLayerDistXY[1]);

//     case ImagingTopoClusterConfig::ELayerMode::xyz:
//       return (std::abs(h1.getPosition().x - h2.getPosition().x) <= sameLayerDistXYZ[0]) &&
//              (std::abs(h1.getPosition().y - h2.getPosition().y) <= sameLayerDistXYZ[1]) &&
//              (std::abs(h1.getPosition().z - h2.getPosition().z) <= sameLayerDistXYZ[2]);
