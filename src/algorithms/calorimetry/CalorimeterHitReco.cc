...
void CalorimeterHitReco::AlgorithmProcess() {
  //const auto rawhits = *m_inputHitCollection;
  //auto hits = *m_outputHitCollection;
  //TODO: convert to JService cellIDPositionConverter when able
...
//        #pragma GCC diagnostic error "-Wsign-converstion"
    ...
    const float energy = (((signed)rh->getAmplitude() - (signed)m_pedMeanADC)) / static_cast<float>(m_capADC) * dyRangeADC / m_sampFrac;
    const float time = rh->getTimeStamp() / stepTDC;

        #pragma GCC diagnostic pop

    const auto cellID = rh->getCellID();
    const int lid = id_dec != nullptr && !m_layerField.empty() ? static_cast<int>(id_dec->get(cellID, layer_idx)) : -1;
    const int sid = id_dec != nullptr && !m_sectorField.empty() ? static_cast<int>(id_dec->get(cellID, sector_idx)) : -1;

    // global positions
    //TODO: convert to JService cellIDPositionConverter when able
    //std::vector<double> gpos{-99.,-99.,-99.};//converter->position(cellID);
    ...
    const auto pos = local.nominal().worldToLocal(dd4hep::Position(1.,2.,3.));//dd4hep::Position(gpos.x, gpos.y, gpos.z)
    ...
    const decltype(eicd::CalorimeterHitData::position) position(gpos.x()/ m_lUnit, gpos.y()/ m_lUnit, gpos.z()/ m_lUnit);
    const decltype(eicd::CalorimeterHitData::dimension) dimension(cdim[0]/ m_lUnit, cdim[1]/ m_lUnit, cdim[2]/ m_lUnit);
    const decltype(eicd::CalorimeterHitData::local) local_position(pos.x()/ m_lUnit, pos.y()/ m_lUnit, pos.z()/ m_lUnit);
