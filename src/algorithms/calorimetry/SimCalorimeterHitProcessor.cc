...
namespace eicrecon{
		...
		for(const auto &[par, hits] : mapMCParToSimCalHit){
			if(is_merge){
				...
				for (const auto &[id, ixs] : merge_map) {
					...
					// attenuation
					double attFactor = 1.;
					if(is_attenuation) attFactor = get_attenuation(leading_hit.getPosition().z);
	...
	edm4hep::MCParticle SimCalorimeterHitProcessor::get_primary(const edm4hep::CaloHitContribution& contrib) const{
