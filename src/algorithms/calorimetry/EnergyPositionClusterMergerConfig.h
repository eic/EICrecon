...
///
/// Serialized format: source:compareSource:zSource:minEnergy:maxEnergy:maxDphi
/// Example: "ec:pc2:pc1:0.5:-1:0.3"
struct PositionRule {
  /// Cluster to use for x/y if this rule fires and candidates agree in phi.
  PositionSource source = PositionSource::pc1;

  /// Cluster to compare against source for the dphi check.
  /// If |dphi(source, compareSource)| > maxDphi, compareSource is used for x/y instead.
  /// Ignored when maxDphi < 0.
  PositionSource compareSource = PositionSource::pc1;

  /// Cluster from which the z coordinate of the position is taken.
  /// x and y come from the source/compareSource/maxDphi logic independently.
  PositionSource zSource = PositionSource::pc1;
  ...
  /// Rule fires only when |dphi(source, compareSource)| > maxDphi. Disabled if < 0.
  double maxDphi = -1;
...
inline std::ostream& operator<<(std::ostream& out, const PositionRule& r) {
  out << r.source << ":" << r.compareSource << ":" << r.zSource << ":"
...
inline std::istream& operator>>(std::istream& in, PositionRule& r) {
  ...
  // is one full rule string, e.g. "ec:pc2:pc1:0.5:-1:0.3".
  ...
  PositionRule tmp;
  if (!(ss >> tmp.source >> tmp.compareSource >> tmp.zSource >>
...
struct EnergyPositionClusterMergerConfig {
  ...
  /// If no rule fires, pc1 is used for all coordinates.
  /// CLI format: comma-separated rules, each rule is source:compareSource:zSource:minEnergy:maxEnergy:maxDphi
  /// Example: -PPrefix:positionRules=ec:pc2:pc1:0.5:-1:0.3,pc2:pc2:pc1:0.5:-1:-1
