...
using ImagingClusterRecoAlgorithm = algorithms::Algorithm<
    algorithms::Input<edm4eic::ProtoClusterCollection, edm4hep::SimCalorimeterHitCollection>,
    ...
    for (const auto& pcl : *proto) {
      ...
      if (mchits->size() > 0) {
        ...
        if (!mchit) {
          ...
          warning("Proto-cluster has highest energy in CellID {}, but no mc hit with that CellID "
                  "was found.",
                  pclhit->getCellID());
          break;
        }
...
private:
  static std::vector<edm4eic::Cluster>
  ...
  fit_track(const std::vector<edm4eic::Cluster>& layers) const {
    int nrows = 0;
