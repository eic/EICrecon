...
void CALOROCDigitization::process(const CALOROCDigitization::Input& input,
  ...
  for (const auto& raw_sample : raw_samples) {
    auto adc = std::clamp(std::llround(raw_sample.adc / m_cfg.dyRangeSingleGainADC * m_cfg.capADC),
                          0LL, static_cast<long long>(m_cfg.capADC) - 1);
    ...
    auto low_adc = std::clamp(std::llround(raw_sample.adc / m_cfg.dyRangeLowGainADC * m_cfg.capADC),
                              0LL, static_cast<long long>(m_cfg.capADC) - 1);
