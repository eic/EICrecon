...
void TOFPulseGeneration::process(const TOFPulseGeneration::Input& input,
  ...
  for (const auto& hit : *simhits) {
...

    {
      ...
      for (double t = tMin, j = 0; t < tMax; ++j, t += interval)
        ADCs[j] +=
            charge * this->_Landau(-m_cfg.gain, t, mpv_analog, m_cfg.sigma_analog) * scalingFactor;
    }
  }
  ...
  for (const auto& [cellID, ADCs] : adc_sum) {
    ...
    time_series.setCharge(
        1.); // placeholder. Don't know what to assign when there are two or more hits
