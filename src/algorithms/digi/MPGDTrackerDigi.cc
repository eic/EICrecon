...
void MPGDTrackerDigi::init() {
  ...
  m_stripRank = [=](CellID vID) {
    ...
    for (rank = 0; rank < 5; rank++)
      if (sID == m_stripIDs[rank]) return rank;
    return -1;
  };
  m_orientation = [=](CellID vID, CellID vJD) {
    int ranki = m_stripRank(vID), rankj = m_stripRank(vJD);
    if      (rankj > ranki) return +1;
    else if (rankj < ranki) return -1;
    else                    return  0;
  };
...
// Interfaces
void getLocalPosMom(const edm4hep::SimTrackerHit &sim_hit,
		    const TGeoHMatrix& toModule,
		    double *lpos, double *lmom);
unsigned int cTraversing(const double *lpos, const double *lmom, double path, bool isSecondary, // Input subHit
			 double rMin, double rMax, // Current instance of SUBVOLUME
			 double dZ, double startPhi, double endPhi, // Module parameters
			 double lins[][3], double louts[][3],
...
unsigned int bTraversing(const double *lpos, const double *lmom, double ref2Cur, double path, bool isSecondary, // Input subHit
...
std::string inconsistency(const edm4hep::EventHeader &event, unsigned int status,
			  CellID cID, const double *lpos, const double *lmom);
std::string oddity(const edm4hep::EventHeader &event, unsigned int status, double dist,
		   CellID cID, const double *lpos, const double *lmom,
		   CellID cJD, const double *lpoj, const double *lmoj);
double outInDistance(int shape, int orientation,
		     double lins[][3], double louts[][3],
...
void MPGDTrackerDigi::process(const MPGDTrackerDigi::Input& input,
  ...
  for (int idx = 0; idx<(int)sim_size; idx++) {
    ...
    // ***** USED HIT?
    int usedHit = 0; for (int jdx : usedHits) {
      if (jdx == idx) { usedHit = 1; break; }
    }
    if (usedHit) continue;
    else usedHits.push_back(idx); // useful?...
    ...
    double lpos[3], lmom[3]; getLocalPosMom(sim_hit,toRefVol,lpos,lmom);
    ...
    if (!strcmp(shape.type(),"TGeoTubeSeg")) {
      ...
      // => Convert it to [-pi,+pi].
      double startPhi = tRef.startPhi()*radian; startPhi -= 2*TMath::Pi();
      double endPhi =   tRef.endPhi()*radian;     endPhi -= 2*TMath::Pi();
	...
	cTraversing(lpos,lmom,sim_hit.getPathLength()*ed2dd,sim_hit.isProducedBySecondary(),
		    rMin,rMax,dZ,startPhi,endPhi,
		    lins,louts,lpini,lpend);
	...
	same_idcs.push_back(idx);
      ...
      bool canReEnter = status & 0x100; int rank = m_stripRank(vID);
      if (!canReEnter) {
	if (rank == 0 && (status & 0x1)) isContinuation =  false;
	if (rank == 0 && (status & 0x2)) hasContinuation = false;
      }
      if (rank == 4 && (status & 0x4))   isContinuation =  false;
      if (rank == 4 && (status & 0x8))   hasContinuation = false;
      if (hasContinuation) {
	// ***** LOOP OVER HITS
	...
	for (jdx = idx+1, unbroken = 1; jdx<(int)sim_size; jdx++) {
	  ...
	  if (!pmoStatus || !isUpstream) {
	    ...
	    if (unbroken) idx = jdx-1; unbroken = 0; continue;
	  }
	  // Get 'j' radii
	  curVol = volman.lookupDetElement(vJD);
	  const Tube &tubj = curVol.solid();
	  rMin = tubj.rMin(); rMax = tubj.rMax();
	  double lpoj[3], lmoj[3]; getLocalPosMom(sim_hjt,toRefVol,lpoj,lmoj);
	  ...
	  if (!jsDownstream) {
	    if (sim_hit.isProducedBySecondary()) break;
	  ...
	  // ***** UPDATE
	  vIDPrv = vJD; eDep += sim_hjt.getEDep();
	  for (int i = 0; i < 3; i++) { // Update end point position/momentum.
	    lpend[i] = lpfnd[i]; lmend[i] = lmoj[i];
	  }
	  // ***** BOOK-KEEPING
	  ...
	  if (!canReEnter && m_stripRank(vJD) == 4) hasContinuation = false;
	  if (!hasContinuation) { jdx++; break; }
	  else { // Update outgoing position/momentum for next iteration.
	    for (int i = 0; i < 3; i++) {
	      louts[0][i] = lovts[0][i]; louts[1][i] = lovts[1][i];
	    }
	  }
	}
	if (unbroken) idx = jdx-1;
      }
      // ***** EXTENSION?...
	...
	if (status & 0xff000) { // Inconsistency => Drop current "sim_hit"
      ...
      flagUnexpected(header,0,(tRef.rMin()+tRef.rMax())/2,
		     sim_hit,lpini,lpend,lpos,lmom);
      // ***** UPDATE (local position <lpos>, DoF
      double DoF2 = 0, dir = 0; for (int i = 0; i<3; i++) {
	double neu = (lpini[i]+lpend[i])/2, alt = lpos[i]; lpos[i] = neu;
        double d = neu-alt; dir += d*lmom[i]; DoF2 += d*d;
      }
      ...
      std::copy(std::begin(lmom), std::end(lmom), std::begin(lmend));
      unsigned int status =
	bTraversing(lpos,lmom,ref2Cur,sim_hit.getPathLength()*ed2dd,sim_hit.isProducedBySecondary(),
		    dZ,dX,dY,
		    lins,louts,lpini,lpend);
      if (status & 0xff000) { // Inconsistency => Drop current "sim_hit"
	critical(inconsistency(header,status,sim_hit.getCellID(),lpos,lmom));
	...
	same_idcs.push_back(idx);
	  ...
	  (rank == 4 && (status & 0x4))) isContinuation =  false;
      if ((rank == 0 && (status & 0x2)) ||
	  (rank == 4 && (status & 0x8))) hasContinuation = false;
      if (hasContinuation) {
	// ***** LOOP OVER SUBHITS
	int jdx, unbroken /* unbroken succession of indices */;
	CellID vIDPrv = vID;
	for (jdx = idx+1, unbroken = 1; jdx<(int)sim_size; jdx++) {
	  ...
	  double ljns[2][3], lovts[2][3], lpjni[3], lpfnd[3];
	  status =
	    bTraversing(lpoj,lmoj,ref2j,sim_hjt.getPathLength()*ed2dd,sim_hit.isProducedBySecondary(),
			dZ,dX,dY,
			ljns,lovts,lpjni,lpfnd);
	  if (status & 0xff000) { // Inconsistency => Drop current "sim_hjt"
	    critical(inconsistency(header,status,sim_hjt.getCellID(),lpoj,lmoj));
	...
	double neu = (lpini[i]+lpend[i])/2, alt = lpos[i]; lpos[i] = neu;
      ...
      critical("Bad input data: CellID {:x} has invalid shape (=\"{}\")",
	       refID,shape.type());
	...
	debug  ("  position  = ({:7.2f},{:7.2f},{:7.2f}) [mm]", globPos.X()/mm, globPos.Y()/mm, globPos.Z()/mm);
      }
	...
	int jdx = same_idcs[ldx]; const edm4hep::SimTrackerHit& sim_hjt = sim_hits->at(jdx);
	...
	debug("  xy_radius = {:.2f}", std::hypot(sim_hjt.getPosition().x, sim_hjt.getPosition().y)/edmm);
    ...
    for (CellID cID : {cIDp, cIDn}) {
      ...
      if (!cell_hit_map.contains(cID)) {
        ...
        hit.setCharge(charge + (std::int32_t)std::llround(eDep * 1e6)); // TODO: accumulate charge: shouldn't it be float?
      }
    }
  }
  ...
  for (auto item : cell_hit_map) {
    ...
    if (is == stripID2cIDs.end()) {
      critical("Inconsistency: CellID {:x} not found in \"stripID2cIDs\" map",
	       stripID);
			 ...
			 double *lpini, double *lpend)
{
  ...
  double ts[3/* rMin/rMax/edge */][2/* In/Out */] = {{0,0},{0,0},{tIn,tOut}}; // Up to two intersections
  double a = Px*Px+Py*Py, b = Px*Mx+Py*My;
  for (int lu = 0; lu < 2; lu++) { // rMin/rMax
    double R; unsigned int statGene;
    if (lu == 1) { R = rMax; statGene = 0x4; }
    else         { R = rMin; statGene = 0x1; }
    double c = M2-R*R;
    if      (!a) { // P is // to Z. Yet no intersect w/ Z edge.
      if ((status & 0x30) != 0x30) status |= 0x1000; continue; // Inconsistency
    }
    else if (!c) { // Hit is on wall: inconsistency.
      status |= 0x2000; continue;
    }
    else {
      double det = b*b-a*c; if (det < 0) {
	if (lu == 1) { // No intersection w/ outer wall: inconsistency.
	  status |= 0x4000; continue;
	}
      }
      else {
	double sqdet = sqrt(det);
	for (int is = 0; is<2; is++) {
	  int s = 1-2*is; double t = (-b+s*sqdet)/a;
	  double Ix = Mx+t*Px, Iy = My+t*Py, Iz = Mz+t*Pz, phi = atan2(Iy,Ix);
	  if (fabs(Iz) > dZ || phi < startPhi || endPhi < phi) continue;
	  if      (t < 0) {
	    ...
	    if (status & statGene) {    // Two <0 ts: can only happen when rMin
	      double tPrv = ts[lu][0]; if (t > tPrv) {
		ts[lu][0] = t; ts[lu][1] = tPrv; // Current is actually IN, previous is OUT despite being <0
	      }
	      status |= statGene<<1;
	    }
	    else { ts[lu][0] = t; status |= statGene; }
	  }
	  else { // (if t > 0)
	    if (status & statGene<<1) { // Two >0 ts: can only happen when rMin
	      double tPrv = ts[lu][1]; if (t < tPrv) {
		ts[lu][1] = t; ts[lu][0] = tPrv; // Current is actually OUT, previous is IN despite being >0
	      }
	      status |= statGene;
	    }
	    else { ts[lu][1] = t; status |= statGene<<1; }
	  }
	}
      }
    }
  ...
  for (int lu = 0; lu < 2; lu++) { // rMin/rMax
    ...
    if (status & statGene) {
      double t = ts[lu][0]; if (t < 0) {
	if (status & 0x10) {
	  if (t < 0 && t > tIn)  status |= 0x10000;
	  status &= ~statGene;
	}
      }
      else { // if (t > 0)
	if (status & 0x20) {
	  if (t > 0 && t < tOut) status |= 0x20000;
	  status &= ~statGene;
	}
      }
    }
    if (status & statGene<<1) {
      double t = ts[lu][1]; if (t < 0) {
	if (status & 0x10) {
	  if (t < 0 && t > tIn)  status |= 0x40000;
	  status &= ~(statGene<<1);
	}
      }
      else { // if (t > 0)
	if (status & 0x20) {
	  if (t > 0 && t < tOut) status |= 0x80000;
	  status &= ~(statGene<<1);
	}
      }
    }
  ...
  if (isReEntering) status |= 0x100; // Remember that particle can re-enter.
  double norm = sqrt(a+Pz*Pz), at = path/2/norm;
  unsigned int statws = 0;
  for (int is = 0; is < 2; is++) {
    int s = 1-2*is; double Ix = s*at*Px, Iy = s*at*Py, Iz = s*at*Pz;
    for (int lu = 0; lu < 2; lu++) { // Lower/upper wall
      unsigned int statvs = lu ? 0x4 : 0x1;
      for (int io = 0; io<2; io++) {
	statvs <<= io; if (status & statvs) {
	  double t = ts[lu][io]; if (t*s < 0) continue;
	  double dIx = t*Px-Ix, dIy = t*Py-Iy, dIz = t*Pz-Iz;
	  double dist = sqrt(dIx*dIx+dIy*dIy+dIz*dIz);
	  const double tolerance = 20*dd4hep::um;
	  if (dist < tolerance) statws |= statvs;
	}
      }
    }
  }
  if (!(statws & 0x5)) /* No entrance */ status &= ~0x5;
  if (!(statws & 0xa)) /* No exit */     status &= ~0xa;
  // ***** End points
  ...
  if (((status & 0x5) == 0x1 || (status & 0x5) == 0x4) && !isSecondary) {
    ...
    lpini[0] = Mx+tIn*Px;  lpini[1] = My+tIn*Py;  lpini[2] = Mz+tIn*Pz;
  }
  else {
    lpini[0] = Mx-at*Px;   lpini[1] = My-at*Py;   lpini[2] = Mz-at*Pz;
  }
  if (((status & 0xa) == 0x2 || (status & 0xa) == 0x8) && !isSecondary) {
    double tOut = (status & 0x2) ? ts[0][1] : ts[1][1];
    lpend[0] = Mx+tOut*Px; lpend[1] = My+tOut*Py; lpend[2] = Mz+tOut*Pz;
  }
  else {
    lpend[0] = Mx+at*Px;   lpend[1] = My+at*Py;   lpend[2] = Mz+at*Pz;
  }
  ...
  for (int lu = 0; lu < 2; lu++) {
    ...
    if (status & statvs) {
      lins [lu][0] = Mx+tIn* Px; lins [lu][1] = My+tIn* Py; lins [lu][2] = Mz+tIn* Pz;
    }
    statvs <<= 1;
    if (status & statvs) {    
      louts[lu][0] = Mx+tOut*Px; louts[lu][1] = My+tOut*Py; louts[lu][2] = Mz+tOut*Pz;
    }
  }
  return status;
}
			 ...
			 double *lpini, double *lpend)
{
  ...
  for (int lu = 0; lu < 2; lu++) {
    int s = 2*lu-1; double Z = s*dZ; unsigned int statGene = lu ? 0x4 : 0x1;
    // Mz+t*Pz = Z
    if (Pz) {
      double t = (Z-Mz)/Pz;
      if      (t < 0) {
	if (!(status & 0x10) || ((status & 0x10) && t > tIn)) {
	  status |= statGene;    tIn =  t;
	}
      }
      else if (t > 0) {
	if (!(status & 0x20) || ((status & 0x20) && t < tOut)) {
	  status |= statGene<<1; tOut = t;
	}
      }
    }
  }
  ...
  for (int is = 0; is < 2; is++) {
    ...
    for (int lu = 0; lu < 2; lu++) { // Lower/upper wall
      ...
      for (int io = 0; io<2; io++) {
	statvs <<= io; if (status & statvs) {
	  double t = io ? tOut : tIn; if (t*s < 0) continue;
	  double dIx = t*Px-Ix, dIy = t*Py-Iy, dIz = t*Pz-Iz;
	  double dist = sqrt(dIx*dIx+dIy*dIy+dIz*dIz);
	  const double tolerance = 20*dd4hep::um;
	  if (dist < tolerance) statws |= statvs;
		  ...
		  double *lext) // Extrapolated position @ <rT>
{
  ...
  if (!c) ok = true;
    ...
    double det = b*b-a*c; if (det >= 0) {
      double sqdet = sqrt(det);
      for (int is = 0; is<2; is++) {
	int s = 1-2*is; double t = (-b+s*sqdet)/a; if (t < 0) continue;
	if (!ok ||
  ...
  if (ok) {
    lext[0] = Mx+tF*Px; lext[1] = My+tF*Py; lext[2] = Mz+tF*Pz;
  }
  return ok;
}
bool bExtrapolate(const double *lpos, const double *lmom, // Input subHit
					 ...
					 double *lext) const
{
  ...
  double t = direction*margin/norm; Mx += t*Px; My += t*Py; Mz += t*Pz; 
  double M2 = Mx*Mx+My*My, rIni = sqrt(M2), rLow, rUp;
  if (rIni < rT) { rLow = rIni; rUp = rT;   }
  else           { rLow = rT;   rUp = rIni; }
  // Intersection w/ the edge in phi
  double tF = 0;
  for (double phi : {startPhi,endPhi}) {
    // M+t*P = 0 + t'*U. t = (My*Ux-Mx*Uy)/(Px*Uy-Py*Ux); 
    double Ux = cos(phi), Uy = sin(phi);
    double D = Px*Uy-Px*Ux; if (D) { // If P not // to U
      double t = (My*Ux-Mx*Uy)/D; if (t*direction < 0) continue;
      double Ex = Mx+t*Px, Ey = My+t*Py, rE = sqrt(Ex*Ex+Ey*Ey), Ez = Mz+t*Pz;
      if (rLow < rE && rE < rUp && fabs(Ez) < dZ) { status |= 0x1; tF = t; }
    }
  }
  // Intersection w/ the edge in Z
  double zLow = -dZ, zUp = +dZ; for (double Z: {zLow,zUp}) {
    // Mz+t*Pz = Z
    if (Pz) {
      double t = (Z-Mz)/Pz; if (t*direction < 0) continue;
      double Ex = Mx+t*Px, Ey = My+t*Py, rE = sqrt(Ex*Ex+Ey*Ey);
      double phi = atan2(Ey,Ex);
      if (rLow < rE && rE < rUp && startPhi < phi && phi < endPhi) {
	if      (t < 0) {
	  if (!status || (status && t > tF))   { status |= 0x1; tF = t; }
	}
	else if (t > 0) {
	  if (!status || (status && t < tF))   { status |= 0x1; tF = t; }
	}
  ...
  if (!status) {
    double a = Px*Px+Py*Py, b = Px*Mx+Py*My, c = M2-rT*rT;
    if      (!a) { // P is // to Z
      if (!status) status |= 0x1000; // Inconsistency
    }
	...
	double sqdet = sqrt(det);
	for (int is = 0; is<2; is++) {
	  int s = 1-2*is; double t = (-b+s*sqdet)/a; if (t*direction < 0) continue;
	  double Ix = Mx+t*Px, Iy = My+t*Py, Iz = Mz+t*Pz, phi = atan2(Iy,Ix);
	  if (fabs(Iz) > dZ && phi < startPhi || endPhi < phi) continue;
	  if (!(status & 0x1) ||
	      // Two intersects: let's retain the earliest one.
					 ...
					 double *lext) const
{
  ...
  for (int xy = 0; xy < 2; xy++) {
    ...
    for (double A: {aLow,aUp}) {
      // Mz+t*Pz = A
      if (Pa) {
	double t = (A-Mz)/Pa; if (t*direction < 0) continue;
	double Eb = Mb+t*Pb, Ez = Mz+t*Pz;
  ...
  if (!status) {
    if (Pz) {
      tF = (zT-Mz)/Pz; if (tF*direction > 0) status = 0x1;
    }
  }
  if (status) {
    lext[0] = Mx+tF*Px; lext[1] = My+tF*Py; lext[2] = Mz+tF*Pz;
...
double getRef2Cur(DetElement refVol, DetElement curVol)
{
  ...
  double gdT[3]; for (int i = 0; i < 3; i++) gdT[i] = TRef[i]-TCur[i];
  double ldT[3]; toRefVol.MasterToLocalVect(gdT,ldT);
  return ldT[2];
}
			  ...
			  CellID cID, const double *lpos, const double *lmom)
{
  ...
  return fmt::format("Event {}#{}, SimHit 0x{:016x} @ {:.2f},{:.2f},{:.2f} mm, P = {:.2f},{:.2f},{:.2f} GeV inconsistency 0x{:x}",
		     event.getRunNumber(),event.getEventNumber(),
		     cID, lpos[0]/mm, lpos[1]/mm, lpos[2]/mm,
		     lmom[0]/GeV, lmom[1]/GeV, lmom[2]/GeV, status);
}
std::string oddity(const edm4hep::EventHeader &event, unsigned int status, double dist,
		   CellID cID, const double *lpos, const double *lmom,
		   CellID cJD, const double *lpoj, const double *lmoj)
{
  using edm4eic::unit::GeV, dd4hep::mm;
  return fmt::format("Event {}#{}, Bizarre SimHit sequence: 0x{:016x} @ {:.4f},{:.4f},{:.4f} mm, P = {:.2f},{:.2f},{:.2f} GeV and 0x{:016x} @ {:.4f},{:.4f},{:.4f} mm, P = {:.2f},{:.2f},{:.2f} GeV: status 0x{:x}, distance {:.4f}",
		     event.getRunNumber(),event.getEventNumber(),
		     cID, lpos[0]/mm, lpos[1]/mm, lpos[2]/mm,
		     lmom[0]/GeV, lmom[1]/GeV, lmom[2]/GeV,
		     cJD, lpoj[0]/mm, lpoj[1]/mm, lpoj[2]/mm,
		     lmoj[0]/GeV, lmoj[1]/GeV, lmoj[2]/GeV,
		     status,dist);
}

bool MPGDTrackerDigi::samePMO(const edm4hep::SimTrackerHit& sim_hit,
			      const edm4hep::SimTrackerHit& sim_hjt,
			      int unbroken) const
{
  // Status:
		     ...
		     double *lmom, double *lmoj)
{
  ...
  bool ok; double lext[3];
  double lmOI[3]; for (int i = 0; i < 3; i++) lmOI[i] = (lmom[i]+lmoj[i])/2;
  double *lOut, *lIn;
  if      (orientation > 0) { lOut = louts[1]; lIn = lins[0]; }
  else if (orientation < 0) { lOut = louts[0]; lIn = lins[1]; }
  else                      { lOut = louts[0]; lIn = lins[0]; }
  ...
  if (ok) {
    double dist2 = 0; for (int i = 0; i < 3; i++) {
      double d = lext[i]-lIn[i]; dist2 += d*d;
    }
    return sqrt(dist2);
  }
  else
...
unsigned int MPGDTrackerDigi::extendHit(CellID refID, int direction,
					double *lpini, double *lmini, double *lpend, double *lmend) const
{
  ...
  if (direction < 0) { lpoE = lpini; lmoE = lmini; }
  else               { lpoE = lpend; lmoE = lmend; }
  ...
  for (int rankE : {0,4}) {
    ...
    if (!strcmp(shape.type(),"TGeoTubeSeg")) {
      ...
      double startPhi = tubE.startPhi()*radian; startPhi -= 2*TMath::Pi();
      double endPhi =   tubE.endPhi()*radian;     endPhi -= 2*TMath::Pi();
      double dZ = tubE.dZ();
      ...
      double Z = rankE == 0 ? -boxE.z() : +boxE.z(); Z -= ref2E;
    ...
    if (direction < 0) {
      for (int i = 0; i < 3; i++) lpini[i] = lext[i];
    }
    else {
      for (int i = 0; i < 3; i++) lpend[i] = lext[i];
    }
    break;
  }
  return status;
...
bool MPGDTrackerDigi::denyExtension(const edm4hep::SimTrackerHit &sim_hit,
				    double depth) const
{
...
void MPGDTrackerDigi::flagUnexpected(const edm4hep::EventHeader &event,
				     int shape, double expected,
				     const edm4hep::SimTrackerHit &sim_hit,
				     double *lpini, double *lpend,
				     double *lpos, double *lmom) const
{
  ...
  double Rnew2 = 0, Znew, diff2 = 0; for (int i = 0; i<3; i++) {
    double neu = (lpini[i]+lpend[i])/2, alt = lpos[i];
    double d = neu-alt; diff2 += d*d;
    if (i != 2) Rnew2 += neu*neu;
    if (i == 2) Znew = neu;
  }
  double found = shape ? Znew : sqrt(Rnew2), residual = found-expected;
  bool isSecondary = sim_hit.isProducedBySecondary();
  bool isPrimary = !isSecondary && sqrt(lmom[0]*lmom[0]+lmom[1]*lmom[1]+lmom[2]*lmom[2]) > .1*GeV;
  if (fabs(residual) > .000001 && isPrimary ||
      sqrt(diff2) >    .000001 && isSecondary) {
    debug("Event {}#{}, SimHit 0x{:016x} origin {:d}: d{:x} = {:.5f} diff = {:.5f}",
	  event.getRunNumber(),event.getEventNumber(),
	  sim_hit.getCellID(),shape?'Z':'R',isSecondary,residual,sqrt(diff2));
  }
}
