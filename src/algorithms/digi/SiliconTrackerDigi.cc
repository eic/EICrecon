...
void SiliconTrackerDigi::process(const SiliconTrackerDigi::Input& input,
  ...
  for (const auto& sim_hit : *sim_hits) {
    ...
    m_log->debug("   position  = ({:.2f}, {:.2f}, {:.2f})", sim_hit.getPosition().x,
                 sim_hit.getPosition().y, sim_hit.getPosition().z);
    m_log->debug("   xy_radius = {:.2f}",
                 std::hypot(sim_hit.getPosition().x, sim_hit.getPosition().y));
    m_log->debug("   momentum  = ({:.2f}, {:.2f}, {:.2f})", sim_hit.getMomentum().x,
    ...
    m_log->debug("   time smearing: {:.4f}, resulting time = {:.4f} [ns]", time_smearing,
                 result_time);
    ...
    if (cell_hit_map.count(sim_hit.getCellID()) == 0) {
      ...
      m_log->debug("  Hit already exists in cell ID={}, prev. hit time: {}", sim_hit.getCellID(),
                   hit.getTimeStamp());
