...
namespace eicrecon {
		...
		for (const auto& pulse : *in_pulses) {
			...
			for (std::size_t i = 0; i < pulse.getAmplitude().size(); i++) {
				...
				if(!totInProgress && amplitude > m_cfg.threshold) {
					toa = get_crossing_time(m_cfg.threshold,
								t, t - pulse.getInterval(),
				...
				if(totInProgress && !totComplete && amplitude < m_cfg.threshold) {
					totComplete = true;
					totInProgress = false;
					tot = (((m_cfg.threshold - amps_neighbor[i % 2]) * pulse.getInterval()) / (amps_neighbor[i % 2] - amps_neighbor[1 - (i % 2)])) + t - toa;	
				}
	...
	double PulseDigi::get_crossing_time(double thres, double t1, double t2, double amp1, double amp2) const {
