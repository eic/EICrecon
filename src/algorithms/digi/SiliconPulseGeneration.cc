...
namespace eicrecon {
	...
	LandauPulse::LandauPulse(std::vector<double> params) {
	...
	double LandauPulse::getMaximumTime() const { return m_hit_sigma_offset * m_sigma_analog; }

	EvaluatorPulse::EvaluatorPulse(const std::string& expression, const std::vector<double>& params) {
	...
	std::unique_ptr<SignalPulse> PulseShapeFactory::createPulseShape(const std::string& type,
