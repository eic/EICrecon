...
void PandoraGeometryMapper::registerGeometry(pandora::Pandora& pandora,
  ...
  for (const auto& spec : kSubDetSpecs) {
    ...
    const DDRec::LayeredCalorimeterData* layerData = nullptr;
    try {
      layerData = det.extension<DDRec::LayeredCalorimeterData>();
    } catch (...) {
...
void PandoraGeometryMapper::registerSubDetector(pandora::Pandora& pandora,
                                                const std::string& subDetName,
                                                pandora::SubDetectorType subDetType, float innerR,
                                                float outerR, float innerZ, float outerZ,
                                                unsigned int nLayers,
                                                const DDRec::LayeredCalorimeterData& layerData) {
  PandoraApi::Geometry::SubDetector subDetParams;
  subDetParams.m_subDetectorName    = subDetName;
  subDetParams.m_subDetectorType    = subDetType;
  subDetParams.m_innerRCoordinate   = innerR;
  subDetParams.m_outerRCoordinate   = outerR;
  subDetParams.m_innerZCoordinate   = innerZ;
  subDetParams.m_outerZCoordinate   = outerZ;
  subDetParams.m_innerPhiCoordinate = 0.f;
  subDetParams.m_outerPhiCoordinate = 0.f;
  subDetParams.m_innerSymmetryOrder = 0;
  subDetParams.m_outerSymmetryOrder = 0;
  // Barrel sub-detectors are mirrored in z; endcaps are not.
  subDetParams.m_isMirroredInZ =
      (subDetType == pandora::ECAL_BARREL || subDetType == pandora::HCAL_BARREL);
  subDetParams.m_nLayers = nLayers;
  ...
  for (unsigned int iLayer = 0; iLayer < nLayers && iLayer < layerData.layers.size(); ++iLayer) {
    ...
    layerParams.m_closestDistanceToIp =
        innerR +
    ...
    layerParams.m_nRadiationLengths   = cumulativeRadLen;
    layerParams.m_nInteractionLengths = cumulativeIntLen;
    subDetParams.m_layerParametersList.push_back(layerParams);
  }
