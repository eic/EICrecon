...
namespace eicrecon {
        ...
        void TrackPropagation::init(std::shared_ptr<const ActsGeometryProvider> geo_svc, std::shared_ptr<spdlog::logger> logger) {
    ...
    ActsTrackPropagationResult TrackPropagation::propagateTrack(const Acts::BoundTrackParameters& params, const std::shared_ptr<const Acts::Surface>& targetSurf) {
        ...
        std::vector<edm4eic::TrackSegment *> TrackPropagation::execute(std::vector<const Jug::Trajectories *> trajectories) {
            // output collection
