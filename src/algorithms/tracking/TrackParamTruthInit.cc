...
void eicrecon::TrackParamTruthInit::init(std::shared_ptr<const ActsGeometryProvider> geo_svc, std::shared_ptr<spdlog::logger> logger) {
    m_log = logger;
    m_geoSvc = geo_svc;
...
eicrecon::TrackParamTruthInit::produce(const edm4hep::MCParticleCollection* mcparticles) {
    ...
    for (const auto& mcparticle: *mcparticles) {
        ...
        auto linesurface_parameter = -(v.x*p.x + v.y*p.y + v.z*p.z)/(p.x*p.x + p.y*p.y + p.z*p.z);
