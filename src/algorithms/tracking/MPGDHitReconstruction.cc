...
void MPGDHitReconstruction::process(const Input& input, const Output& output) const {
  ...
  size_t idx(0); std::generate(idcs.begin(), idcs.end(), [&]{ return idx++; });
  std::sort(idcs.begin(), idcs.end(),
	    [&](int idxa, int idxb) { return raw_hits->at(idxa).getCellID() < raw_hits->at(idxb).getCellID(); });
  ...
  std::vector<double> clusDim(3);   // cc: uncertainty = resolution along measurement axis, else weighted dimension
  double clusCharge;                // cc: sum of charges
  double sW;                        // cc: sum of Weights (= "clusCharge", as of 3025/11)
  double clusChMx; int clusChMxIdx; // cc: Max. charge and index of: determine timing
  int clusFirstIdx;                 // cc: Index of 1st rawHit contributing
  // Loop on ordered raw_hits + a last iteration to store last cluster
  int jdx; for (jdx = 0, prvID = 0, currentPN = -1, sW = 0; jdx <= nRawHits; jdx++){
    bool lastHit = jdx == nRawHits;
    bool newCluster; CellID cID; int pn, idx;
    if (!lastHit){
      idx = idcs[jdx];
      const auto &raw_hit = raw_hits->at(idx);
      cID = raw_hit.getCellID();
      if      (cID&m_pStripBit) pn = 0;
      else if (cID&m_nStripBit) pn = 1;
      else {
	critical(R"(Invalid RawHit cellID (=0x{:0>16x}) from "{}" readout.)",
		 cID, m_cfg.readout);
	...
	clusPos += m_converter->position(cID) * weight; sW += weight;
	std::vector<double> dim = m_converter->cellDimensions(cID);
	size_t nDims = std::size(dim);
	for (size_t i = 0; i < nDims; i++) clusDim[i] += dim[i] * weight;
	clusCharge += weight;
	if (weight > clusChMx){
	  clusChMxIdx = idx; clusChMx = weight;
	}
	if (idx < clusFirstIdx){
	  clusFirstIdx = idx;
	}
	if (nDims != currentNDims){
	  critical(R"(RawHits from "{}" readout have different #dims: cellID 0x{:0>16x} = %u, cellID 0x{:0>16x} = %u.)",
	...
	clusDim[i] /= mm; clusDim[i] *= clusDim[i];
      }
      // >oO trace
    ...
    if (newCluster) {
      ...
      clusChMx = weight;
      clusChMxIdx = idx;
      currentPN = pn; currentNDims = nDims;
      prvID = cID;
      clusFirstIdx = idx;
    }
  }
