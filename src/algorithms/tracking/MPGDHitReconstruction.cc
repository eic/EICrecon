...
void MPGDHitReconstruction::process(const Input& input, const Output& output) const {
  ...
  size_t idx(0);
  std::ranges::generate(idcs, [&] { return idx++; });
  std::ranges::sort(idcs, [&](int idxa, int idxb) {
    CellID cIDa = raw_hits->at(idxa).getCellID();
    CellID vIDa = cIDa & m_subVolBits;
    CellID cIDb = raw_hits->at(idxb).getCellID();
    CellID vIDb = cIDb & m_subVolBits;
    if (vIDa != vIDb) {
      return vIDa < vIDb;
    }
    int pna = (vIDa & m_pStripBit) ? 0 : 1;
    int pnb = (vIDb & m_pStripBit) ? 0 : 1;
    if (pna != pnb) {
      return pna < pnb;
    }
  ...
  CellID prvID = 0; // CellID of previous
  // Current cluster (cc): in the making or to be stored
  int currentPN       = 0;
  size_t currentNDims = 0;
  ...
  for (jdx = 0, prvID = 0, currentPN = -1, sW = 0; jdx <= nRawHits; jdx++) {
    ...
    if (!lastHit) {
      ...
      if ((prvID != 0u) && !newCluster) {
        ...
        clusFirstIdx = std::min(idx, clusFirstIdx);
    ...
    if (prvID != 0u) {
      ...
      clusPos /= sW * mm;
      // Variance
      for (int i = 0; std::cmp_less(i, currentNDims); i++) {
    ...
    if (newCluster) {
      ...
      clusChMx     = weight;
      clusChMxIdx  = idx;
      currentPN    = ((cID & m_pStripBit) != 0u) ? 0 : 1;
      currentNDims = nDims;
      prvID        = cID;
      clusFirstIdx = idx;
    }
...
void MPGDHitReconstruction::parseIDDescriptor() {
  ...
  for (int pn = 0; pn < 2; pn++) {
    ...
    m_coordOffsets[pn]                  = std::min(offset, m_coordOffsets[pn]);
  }
