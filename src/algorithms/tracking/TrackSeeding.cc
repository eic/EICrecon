...
eicrecon::TrackSeeding::produce(const edm4eic::TrackerHitCollection& trk_hits) {
  ...
  std::function<std::pair<Acts::Vector3, Acts::Vector2>(const eicrecon::SpacePoint* sp)>
...
eicrecon::TrackSeeding::makeTrackParams(SeedContainer& seeds) {
  ...
  for (auto& seed : seeds) {
    ...
    trackparam.setMomentumError(
        {m_cfg.thetaError, m_cfg.phiError, m_cfg.qOverPError}); // covariance on theta/phi/q/p
