...
void AudioAnomalyDetection_service::initializeAudio() {
  ...
  if (rc < 0) {
    m_log->warn("Unable to open PCM device {}: {} - running in silent mode", m_audio_device,
                snd_strerror(rc));
  ...
  m_log->info("Audio initialized: {} Hz, {} channels, {} samples buffer", sample_rate, m_channels,
              frames);
}
...
void AudioAnomalyDetection_service::reportAnomaly(const std::string& detector_name,
                                                  double anomaly_level) {
...
bool AudioAnomalyDetection_service::isStreamActive() const { return m_running.load(); }
