...
void JEventSourcePODIO::GetEvent(std::shared_ptr<JEvent> event) {
  ...
  const auto& event_headers = frame->get<edm4hep::EventHeaderCollection>(
      "EventHeader"); // TODO: What is the collection name?
  if (event_headers.size() != 1) {
    throw JException("Bad event headers: Entry %d contains %d items, but 1 expected.", Nevents_read,
                     event_headers.size());
  }
