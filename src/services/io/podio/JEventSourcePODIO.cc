...
bool JEventSourcePODIO::IsOpenable(const std::string& filename) {
  ...
  std::unique_ptr<TFile> file = std::unique_ptr<TFile>{TFile::Open(filename.c_str())};
...
double JEventSourceGeneratorT<JEventSourcePODIO>::CheckOpenable(std::string resource_name) {
  return JEventSourcePODIO::IsOpenable(resource_name) ? 0.03 : 0.0;
}
