...
private:
  ...
  void addDataTypeToMap() {
    addToMap<typename DataT::collection_type>();
  }
...
struct VisitPodioCollection {
  void operator()(Visitor& visitor, const podio::CollectionBase& collection) {
    ...
    if (it != visitorMap.getMap().end()) {
      ...
      throw std::runtime_error(std::string("Unrecognized podio typename: ") + std::string(typeName));
    }
  }
};
