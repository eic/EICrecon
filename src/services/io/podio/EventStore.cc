...
EventStore::EventStore() : m_table(new podio::CollectionIDTable()) {
...
bool EventStore::doGet(const std::string& name, podio::CollectionBase*& collection, bool setReferences) const {
...
podio::GenericParameters& EventStore::getEventMetaData() {
  ...
  if (m_evtMD.empty() && m_reader != nullptr) {
    podio::GenericParameters* tmp = m_reader->readEventMetaData();
...
bool EventStore::collectionRegistered(const std::string& name) const {
  auto result = std::find_if(std::begin(m_collections), std::end(m_collections),
                             [&name](const CollPair& item) -> bool { return name == item.first; });
  return (result != std::end(m_collections));
}
